 ...
 *
 *   Compile with:
 *     $> gcc -o css-bundle-watchdog css-bundle-watchdog.c
 *
 *   Run as:
 *     $> ./css-bundle-watchdog -o {bundle-file.css} /path/to/monitor /another/path/to/monitor ...
 */
...
void refresh(const char *path, FILE *outfile){
    ...
    while ((de = readdir(dir))) {     // Fill one files.[filename] per file.
        ...
        if (l < 4 || 0 != strcmp(".css", &de->d_name[l-3])){
            continue;
        }
...
__event_process (struct inotify_event *event)
{
    ...
    // Just lock for *.css files and abort otherwise
    size_t l = strlen(fname);
    if (l >= 4 && 0 == strcmp(".css", &fname[l-3])){
        event_can_be_skiped = 0;
    }
...
int main (int argc, const char **argv)
{
    ...
    struct argparse_option options[] = {
        ...
        //OPT_BOOLEAN('c', "uglifycss", &uglifycss, "Compress output with uglifycss", NULL, 0, 0),
        OPT_STRING('c', "uglifycss", &uglifycss_args, "Compress output with uglifycss", NULL, 0, 0),
