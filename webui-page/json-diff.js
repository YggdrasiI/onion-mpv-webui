...
function get_diff(new_list, old_list, compare_handler){
  ...
  while (iNew<nNew && iOld<nOld) {
    ...
    if (new_list[iNew+1] && old_list[iOld+1] &&
      ...
      operations[operations.length] = {'op': 'replace', 'new': iNew, 'old': iOld}
      iNew+=2
      iOld+=2
      continue
    }
    ...
    if (new_list[iNew+1] &&
      ...
      operations[operations.length] = {'op': 'add', 'new': iNew, 'old': iOld}
      iNew+=2
      iOld+=1
      continue
    }
    ...
    if (old_list[iOld+1] &&
      ...
      operations[operations.length] = {'op': 'remove', 'new': -1, 'old': iOld}
      iNew+=1
      iOld+=2
      continue
    }
